...
export default {
  ...
  methods: {
    ...
    async toSearch(item) {
      ...
      let { totalRow, dataList = [], rescode } = await this.entTypeOperation({ data: newObject, type: 'entTypeList' });
    ...
    async operation() {
      ...
      let type;
      has_id > 0 ? (type = 'entTypeEdit') : (type = 'entTypeSave');
      let result = await this.entTypeOperation({ data: this.form, type: type });
      this.toSearch();
      this.closeAlert();
    },
      ...
      if (type === 'edit') {
        ...
        await this.$confirm('确认要删除该数据吗?', `删除提示`, {
          ...
          .then(async () => {
            ...
            await this.entTypeOperation({ data: { id: this.operationId }, type: 'entTypeDelete' });
            this.closeAlert();
            this.toSearch();
          })
          .catch(() => {
